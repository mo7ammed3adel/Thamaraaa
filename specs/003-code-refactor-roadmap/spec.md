...
- [x] R0322 إنشاء `src/server/services/taskWorkflowService.ts` ونقل update/reassign/flag/generate task flows.
...
- [x] R0328 إنشاء `src/server/services/analyticsService.ts` لتجميع analytics queries المتكررة.
...
- [x] R0425 Replace fetch in Sales/TeleSales only after smoke test baseline.
...
- [x] R0511 استخراج `ClientTimelineTab`.
- [x] R0512 استخراج `ClientTasksTab`.
...
- [x] R0517 تقليل `ClientFullJourneyClient.tsx` إلى orchestrator لا يتخطى 250-300 سطر.
