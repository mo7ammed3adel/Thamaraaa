          ...
          <div className="bg-white rounded-2xl border shadow-sm overflow-hidden">
            ...
            <div className="p-4 space-y-3">
              <form onSubmit={addDoc} className="grid grid-cols-12 gap-2">
                ...
                <button type="submit" disabled={busy} className="col-span-2 bg-slate-900 text-white rounded-lg text-xs font-bold disabled:opacity-50">Add</button>
              </form>
              {loading ? <p className="text-sm text-slate-400">Loading…</p> : (
                <div className="space-y-1.5">
                  {docs.length === 0 && <p className="text-sm text-slate-400 italic">No files uploaded yet.</p>}
                  {docs.map((d) => (
                    <div key={d.id} className="flex items-center justify-between border rounded-lg px-3 py-2 text-sm">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => removeDoc(d.id)} className="text-red-500" title="Delete"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </div>
                  ))}
                </div>
