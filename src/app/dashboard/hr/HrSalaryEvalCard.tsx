    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-slate-50 border rounded-xl p-3"><p className="text-[11px] font-bold uppercase text-slate-400">{t("hr.nextIncrease")}</p><p className="text-sm font-bold text-slate-800">{info.increaseType === "fixed" ? money(info.increaseValue) : `${info.increaseValue || 0}%`}</p><p className="text-[10px] text-slate-400">min eval {info.minEvalForIncrease || 0}%</p></div>
        </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        ...
        {!info.commission?.enabled ? (
          <p className="text-sm text-slate-400 italic">{t("hr.noCommissionScheme")}</p>
        ) : (
          <>
            <p className="text-xs text-slate-500 mb-2">{t("hr.typeLabel")} <span className="font-bold capitalize">{info.commission.type}</span> (inherited from your department)</p>
            {rules.length === 0 ? (
              <p className="text-sm text-slate-400 italic">{t("hr.noTiersConfigured")}</p>
            ) : (
              <ul className="space-y-1.5">
                {rules.map((r, i) => (
                  <li key={i} className="flex items-center justify-between text-sm border rounded-lg px-3 py-2">
                    <span className="text-slate-600">{t("hr.salesAtLeast")} <span className="font-semibold">{Number(r.minSales).toLocaleString()}</span></span>
