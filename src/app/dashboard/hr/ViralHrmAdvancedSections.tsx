...
export function Workflows({ workflows, requestTypes, busy, onCreate, onUpdate, onDelete }: any) {
  ...
  return (
    <TwoColumn>
      <DataPanel title={t("hrm.workflowBuilder")}>
        <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); onCreate("workflow", workflowValues(e.currentTarget), e.currentTarget); }}>
          ...
          <PrimaryButton disabled={busy}>{t("hrm.createWorkflow")}</PrimaryButton>
        </form>
      </DataPanel>
      <DataPanel title={t("hrm.configuredWorkflows")}>
        <SimpleTable
          ...
          rows={workflows.map((workflow: any) => [
            ...
            <div key={workflow.id} className="flex gap-2">
              ...
              <button onClick={() => onDelete("workflow", workflow.id)} className="text-xs font-bold text-red-600">{t("common.delete")}</button>
            </div>,
          ])}
        />
      </DataPanel>
...
export function RequestCenter({ employees, requestTypes, requests, delegations, tags, busy, onCreate, onUpdate, onDelete }: any) {
  ...
  return (
    <div className="space-y-5">
      <TwoColumn>
        <DataPanel title={t("hrm.requestTypesSla")}>
          <form className="grid grid-cols-1 md:grid-cols-2 gap-3" onSubmit={(e) => { e.preventDefault(); onCreate("requestType", formValues(e.currentTarget), e.currentTarget); }}>
            ...
            <div className="md:col-span-2"><PrimaryButton disabled={busy}>{t("hrm.createType")}</PrimaryButton></div>
          </form>
        </DataPanel>
        <DataPanel title={t("hrm.submitCentral")}>
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); onCreate("hrRequest", requestValues(e.currentTarget), e.currentTarget); }}>
            ...
            <PrimaryButton disabled={busy}>{t("hrm.submitRequest")}</PrimaryButton>
          </form>
        </DataPanel>
      </TwoColumn>
      <DataPanel title={t("hrm.requestsStatus")}>
        ...
        <SimpleTable
          ...
          rows={requests.map((request: any) => [
            ...
            <div key={request.id} className="flex flex-wrap gap-2">
              ...
              <button onClick={() => onDelete("hrRequest", request.id)} className="text-xs font-bold text-red-600">{t("status.archive")}</button>
            </div>,
          ])}
        />
      </DataPanel>
      <TwoColumn>
        <DataPanel title={t("hrm.delegation")}>
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); onCreate("delegation", formValues(e.currentTarget), e.currentTarget); }}>
            ...
            <PrimaryButton disabled={busy}>{t("hrm.createDelegation")}</PrimaryButton>
          </form>
        </DataPanel>
        <DataPanel title={t("hrm.tags")}>
          <form className="flex gap-2 mb-3" onSubmit={(e) => { e.preventDefault(); onCreate("requestTag", formValues(e.currentTarget), e.currentTarget); }}>
            <input name="name" required placeholder={t("hrm.tagName")} className="flex-1 border rounded-lg px-3 py-2 text-sm" />
            <input name="color" placeholder="#64748b" className="w-28 border rounded-lg px-3 py-2 text-sm" />
            <button disabled={busy} className="px-3 py-2 bg-slate-900 text-white rounded-lg text-sm font-bold">{t("common.add")}</button>
          </form>
          <SimpleTable columns={["Name", "Color"]} rows={tags.map((tag: any) => [tag.name, tag.color || "-"])} />
        </DataPanel>
      </TwoColumn>
      <DataPanel title={t("hrm.activeDelegations")}>
...
export function AttendanceOps({ employees, shifts, shiftAssignments, holidays, attendanceAdjustments, busy, onCreate, onUpdate, onDelete }: any) {
  ...
  return (
    <div className="space-y-5">
      <TwoColumn>
        <DataPanel title={t("hr.createShift")}>
          <form className="grid grid-cols-2 gap-3" onSubmit={(e) => { e.preventDefault(); onCreate("shift", formValues(e.currentTarget), e.currentTarget); }}>
            ...
            <div className="col-span-2"><PrimaryButton disabled={busy}>{t("hr.createShift")}</PrimaryButton></div>
          </form>
        </DataPanel>
        <DataPanel title={t("hrm.officialHoliday")}>
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); onCreate("holiday", formValues(e.currentTarget), e.currentTarget); }}>
            ...
            <PrimaryButton disabled={busy}>{t("hrm.createHoliday")}</PrimaryButton>
          </form>
        </DataPanel>
      </TwoColumn>
      <DataPanel title={t("hrm.attendanceAdjustments")}>
        <form className="grid grid-cols-1 md:grid-cols-6 gap-2 mb-4" onSubmit={(e) => { e.preventDefault(); onCreate("attendanceAdjustment", formValues(e.currentTarget), e.currentTarget); }}>
          ...
          <Field name="reason" label="Reason" required />
          <div className="md:col-span-6"><PrimaryButton disabled={busy}>{t("hrm.submitAdjustment")}</PrimaryButton></div>
        </form>
        <SimpleTable
          ...
          rows={attendanceAdjustments.map((adjustment: any) => [
            ...
            <div key={adjustment.id} className="flex gap-2">
              <button onClick={() => onUpdate("attendanceAdjustment", { id: adjustment.id, status: "approved" })} className="text-xs font-bold text-emerald-700">{t("common.approve")}</button>
              <button onClick={() => onUpdate("attendanceAdjustment", { id: adjustment.id, status: "rejected" })} className="text-xs font-bold text-red-600">{t("common.reject")}</button>
            </div>,
          ])}
        />
      </DataPanel>
      <TwoColumn>
        <DataPanel title={t("hrm.shifts")}><SimpleTable columns={["Name", "Time", "Grace", "Status"]} rows={shifts.map((shift: any) => [shift.name, `${shift.startTime} - ${shift.endTime}`, `${shift.lateGraceMinutes} min`, statusBadge(shift.active ? "active" : "inactive")])} /></DataPanel>
        <DataPanel title={t("hrm.holidays")}><SimpleTable columns={["Name", "Dates", "Applies"]} rows={holidays.map((holiday: any) => [holiday.name, `${formatDate(holiday.startDate)} - ${formatDate(holiday.endDate)}`, holiday.appliesTo])} /></DataPanel>
      </TwoColumn>
    </div>
  );
}
...
export function TalentPool({ departments, candidates, processes, interviews, offers, busy, onCreate, onUpdate, onDelete }: any) {
  ...
  return (
    <div className="space-y-5">
      <TwoColumn>
        <DataPanel title={t("hr.addCandidate")}>
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); onCreate("candidate", formValues(e.currentTarget), e.currentTarget); }}>
            ...
            <PrimaryButton disabled={busy}>{t("hr.addCandidate")}</PrimaryButton>
          </form>
        </DataPanel>
        <DataPanel title={t("hrm.reuseCandidate")}>
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); onCreate("candidateProcess", formValues(e.currentTarget), e.currentTarget); }}>
            ...
            <PrimaryButton disabled={busy}>{t("hrm.startProcess")}</PrimaryButton>
          </form>
        </DataPanel>
      </TwoColumn>
      <DataPanel title={t("hrm.talentPool")}>
        <SimpleTable columns={["Candidate", "Phone", "Current", "Processes", "Actions"]} rows={candidates.map((candidate: any) => [
          ...
          <button key={candidate.id} onClick={() => onDelete("candidate", candidate.id)} className="text-xs font-bold text-red-600">{t("status.archive")}</button>,
        ])} />
      </DataPanel>
      <TwoColumn>
        <DataPanel title={t("hrm.scheduleInterview")}>
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); onCreate("interview", formValues(e.currentTarget), e.currentTarget); }}>
            ...
            <PrimaryButton disabled={busy}>{t("lead.schedule")}</PrimaryButton>
          </form>
        </DataPanel>
        <DataPanel title={t("hr.createOffer")}>
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); onCreate("offer", formValues(e.currentTarget), e.currentTarget); }}>
            ...
            <PrimaryButton disabled={busy}>{t("hr.createOffer")}</PrimaryButton>
          </form>
        </DataPanel>
      </TwoColumn>
      <DataPanel title={t("hrm.recruitmentProcesses")}>
...
export function CompensationCenter({ employees, salaryChanges, promotions, compensationItems, busy, onCreate, onUpdate }: any) {
  ...
  return (
    <div className="space-y-5">
      <TwoColumn>
        <DataPanel title={t("hrm.salaryChange")}>
        ...
        <DataPanel title={t("hrm.promotionTransfer")}>
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); onCreate("promotion", formValues(e.currentTarget), e.currentTarget); }}>
            ...
            <Field name="effectiveDate" label="Effective date" type="date" required />
            <Field name="reason" label="Reason" required />
            <PrimaryButton disabled={busy}>{t("hrm.createPromotion")}</PrimaryButton>
          </form>
        </DataPanel>
      </TwoColumn>
      <DataPanel title={t("hrm.salaryApproval")}>
        <SimpleTable columns={["Employee", "Type", "Current", "Proposed", "Diff", "Status", "Actions"]} rows={salaryChanges.map((request: any) => [
          ...
          <div key={request.id} className="flex gap-2">
            <button onClick={() => onUpdate("salaryChange", { id: request.id, status: "approved" })} className="text-xs font-bold text-emerald-700">{t("common.approve")}</button>
            <button onClick={() => onUpdate("salaryChange", { id: request.id, status: "rejected" })} className="text-xs font-bold text-red-600">{t("common.reject")}</button>
          </div>,
        ])} />
      </DataPanel>
      <TwoColumn>
        <DataPanel title={t("hrm.bonusAllowanceDeduct")}>
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); onCreate("compensationItem", formValues(e.currentTarget), e.currentTarget); }}>
            ...
            <PrimaryButton disabled={busy}>{t("hrm.addItem")}</PrimaryButton>
          </form>
        </DataPanel>
        <DataPanel title={t("hrm.promotionApprovals")}>
          <SimpleTable columns={["Employee", "Type", "New role", "Status", "Actions"]} rows={promotions.map((promotion: any) => [
            ...
            <button key={promotion.id} onClick={() => onUpdate("promotion", { id: promotion.id, status: "approved" })} className="text-xs font-bold text-emerald-700">{t("common.approve")}</button>,
          ])} />
        </DataPanel>
      </TwoColumn>
      <DataPanel title={t("hrm.compensationItems")}>
...
export function AuditTrail({ activityLogs, settingAudits }: any) {
  ...
  return (
    <TwoColumn>
      <DataPanel title={t("hrm.activityLog")}>
      ...
      <DataPanel title={t("hrm.settingsAudit")}>
