...
export default function MediaBuyerClient({ projects, teamMembers, userRole, userId }: any) {
  ...
  const isDeptTask = (t: any) => DEPT_TYPES.includes(t.taskType);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6">
        {filteredProjects.map((project: any) => {
          ...
          const deptProgress = project.mediaBuyerProgress || 0;
          ...
          return (
            <div key={project.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                  ...
                  {isTL && (
                    <div className="mt-2 flex items-center gap-2 max-w-xs">
                      <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-2 bg-amber-500 rounded-full" style={{ width: `${Math.round(deptProgress)}%` }} />
                      </div>
                      <span className="text-[11px] font-black text-amber-700">{Math.round(deptProgress)}%</span>
                    </div>
                  )}
                </div>
              ...
              {/* TL View: Show Agents & Tasks + Self-Task */}
              {isTL && (
                            ...
                            <div key={ta.id} className="border rounded-lg p-3">
                              ...
                              <div className="space-y-2 mt-3">
                                {agentTasks.map((task: any) => (
                                  <div key={task.id} className={`text-xs p-2 rounded flex justify-between items-center gap-2 border ${task.status === "review" ? "bg-amber-50 border-amber-200" : "bg-slate-50"}`}>
                                    <span className="font-medium truncate flex-1">
                                      {task.taskType.replace(/_/g, " ")}
                                      {task.status === "review" && <span className="ms-1 text-amber-600 font-bold">· needs review</span>}
                                    </span>
                                    <select
                                      value={task.status}
