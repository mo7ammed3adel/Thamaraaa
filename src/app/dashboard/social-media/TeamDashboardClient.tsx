  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className={`p-4 rounded-xl border text-center ${colors.stat}`}><p className="text-xs font-medium">Total</p><p className="text-2xl font-bold">{tasks.length}</p></div>
        ...
        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 text-center"><p className="text-xs text-blue-600">Review</p><p className="text-2xl font-bold text-blue-700">{tasks.filter((t: any) => t.status === "review").length}</p></div>
        <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 text-center"><p className="text-xs text-emerald-600">Done</p><p className="text-2xl font-bold text-emerald-700">{tasks.filter((t: any) => t.status === "done").length}</p></div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {tasks.length === 0 && <p className="text-sm text-slate-400 italic text-center py-8">No tasks yet.</p>}
      </div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <select value={subTaskLeader} onChange={(e) => setSubTaskLeader(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm">
                  ...
                  {designLeaders.filter((d: any) => {
                    const st = subTaskTypes.find((s) => s.value === subTaskType);
                    return st ? d.role === st.leaderRole : true;
