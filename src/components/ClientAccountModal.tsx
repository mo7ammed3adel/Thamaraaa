...
export default function ClientAccountModal({
  ...
  const loadAccount = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof HttpError ? err.message : t("portal.loadFailed"));
    } finally {
      setLoading(false);
    }
  }, [leadId]);
  ...
  async function run(action: () => Promise<void>) {
    ...
    try {
      ...
      setError(err instanceof HttpError ? err.message : t("portal.actionFailed"));
    } finally {
      setBusy(false);
    }
  }
        ...
        <div className="space-y-4 px-6 py-5">
          ...
          {loading ? (
            ...
            <>
              ...
              >
                {busy ? t("portal.creating") : t("portal.createAccount")}
              </button>
            </>
          ) : (
            <>
              <dl className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm">
                <div className="flex justify-between">
                  <dt className="text-slate-500">{t("portal.username")}</dt>
                  <dd dir="ltr" className="font-bold text-slate-800">{account.username}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-slate-500">{t("portal.accountStatus")}</dt>
                  <dd
                    className={`font-bold ${account.status === "Active" ? "text-emerald-600" : "text-red-600"}`}
                  >
                    {account.status === "Active" ? t("portal.enabled") : t("portal.suspended")}
                  </dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-slate-500">{t("portal.lastLogin")}</dt>
                  <dd className="font-medium text-slate-700">
                    {account.lastLoginAt ? formatDateTime(account.lastLoginAt) : t("portal.neverLoggedIn")}
                  </dd>
                </div>
              </dl>
              ...
              {temporaryPassword && (
                <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                  ...
                  >
                    ...
                    {copied ? t("portal.copied") : t("portal.copyCredentials")}
                  </button>
                </div>
              )}
              ...
              <div className="flex gap-2">
                ...
                >
                  {account.status === "Active" ? t("portal.disableAccess") : t("portal.enableAccess")}
                </button>
              </div>
            </>
          )}
